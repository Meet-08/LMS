...
const UserDashboard = () => {
  const { data: userBorrowedBooks } = useGetUserBorrowedBooksQuery();

  const [totalBorrowedBooks, setTotalBorrowedBooks] = useState(0);
  const [totalReturnedBooks, setTotalReturnedBooks] = useState(0);

  useEffect(() => {
    let totalBorrowed = userBorrowedBooks?.filter(
      (book) => !book.returned
    ).length;

    let totalReturned = userBorrowedBooks?.filter(
      (book) => book.returned
    ).length;
          ...
          <div className="flex flex-[4] flex-col gap-7 lg:py-5 justify-center xl:min-h-[85.5vh]">
            <div className="flex flex-col gap-7 flex-[4]">
              <div className="flex flex-col lg:flex-row gap-7 overflow-hidden">
                <div className="flex items-center p-5 gap-3 bg-white max-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                ...
                <div className="flex items-center p-5 gap-3 bg-white min-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                  <span className="w-[2px] bg-black h-20 lg:h-full"></span>
                  <span className="bg-gray-300 h-20 lg:h-full min-w-20 flex justify-center items-center rounded-lg">
                    <img
                      src={returnIcon}
              ...
              <div className="flex flex-col lg:flex-row gap-7">
                <div className="flex items-center p-5 gap-3 bg-white min-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                  <span className="w-[2px] bg-black h-20 lg:h-full"></span>
                  <span className="bg-gray-300 h-20 lg:h-full min-w-20 flex justify-center items-center rounded-lg">
                    <img src={browseIcon} alt="browseIcon" className="size-8" />
                  </span>
                  <p className="text-lg xl:text-xl font-semibold">
